...
def set_task(title, data):
    r.set(f"task:{title}", json.dumps(data))
...
def get_title_from_url(base_url, remote_addr):
    ...
    try:
        ...
        meta_url = f"https://www.redbull.com/v3/api/graphql/v1/v3/feed/{selected_locale}?disableUsageRestrictions=true&filter[uriSlug]={url.split('/')[-1]}&rb3Schema=v1:pageConfig&rb3PageUrl=/{url}"
